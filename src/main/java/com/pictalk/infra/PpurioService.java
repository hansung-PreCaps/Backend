...
public class PpurioService {
    ...
    public ResponseEntity<Map> sendMMS(List<FileDto> files, Sender sender, String accessToken, SendMessageRequest request) {
        ...
        PpurioRequestDto.PpurioMMSRequestDto ppurioMMSRequestDto = PpurioRequestDto.PpurioMMSRequestDto.builder()
                ...
                .messageType(request.getMessageType().toUpperCase())
                ...
                .sendTime(request.getSendTime() == null ? null : request.getSendTime())
                .subject(request.getSubject())
                .files(files == null ? null : files.toArray(new FileDto[files.size()]))
                .build();
    ...
    public ResponseEntity<Map> sendKakao(List<Resend> resends, Sender sender, String accessToken, SendKakaoRequest request) {
        ...
        PpurioRequestDto.PpurioKakaoRequestDto ppurioKakaoRequestDto = PpurioRequestDto.PpurioKakaoRequestDto.builder()
                ...
                .resend(request.getIsResend() == "N" || resends == null ? null : resends.get(0))
                .build();
    ...
    public ResponseEntity<Map> cancelMessage(String accessToken, String messageKey) {

        HttpHeaders messageHeaders = new HttpHeaders();
        messageHeaders.setContentType(MediaType.APPLICATION_JSON);
        messageHeaders.setBearerAuth(accessToken);

        PpurioRequestDto.PpurioCancelRequestDto ppurioCancelRequestDto = PpurioRequestDto.PpurioCancelRequestDto.builder()
                .account(ppurioClient.getPpurioAccount())
                .messageKey(messageKey)
                .build();

        HttpEntity<PpurioRequestDto.PpurioCancelRequestDto> messageEntity = new HttpEntity<>(ppurioCancelRequestDto, messageHeaders);

        // 6. 메시지 전송 및 응답 처리
        ResponseEntity<Map> messageResponse = restTemplate.exchange(ppurioClient.getApiUrl() + "/v1/cancel",
                HttpMethod.POST,
                messageEntity,
                Map.class);
