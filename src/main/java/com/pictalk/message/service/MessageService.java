...
import com.pictalk.message.domain.Request;
...
public class MessageService {
    ...
    public void setFROM(String senderNumber) {
        this.FROM = senderNumber;
    }

    public void setFILE_PATH(String imageUrl) {
        this.FILE_PATH = imageUrl;
    }


    public String requestSend(MessageRequestDto.CommonMessage commonMessage) {
        ...
        Map<String, Object> sendResponse = send(URI, (String) tokenResponse.get("token"), commonMessage); // 발송 요청

        System.out.println(sendResponse.toString()); // 토큰 발급
        // 발송 요청
    ...
    private Map<String, Object> send(String baseUri, String accessToken, MessageRequestDto.CommonMessage commonMessage) {
        HttpURLConnection conn = null;
        try {
            String bearerAuthorization = "Bearer " + accessToken;
            Request request = new Request(baseUri + "/v1/message", bearerAuthorization);

            conn = createConnection(request, commonMessage); // 메시지 전송 파라미터 생성
    ...
    private <T> HttpURLConnection createConnection(Request request, T requestObject) throws IOException {
        ...
        HttpURLConnection connect = createConnection(request, jsonInputString);
